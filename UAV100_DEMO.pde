...
void demofunction() {
  us32 i = 0;
  us8 j = 0;
  double midpoint = ((float)ram.upperlimit + (float)ram.lowerlimit)/2;
  double swing = ((float)ram.upperlimit - (float)ram.lowerlimit)/2 -5000;
  for(j=0;j<3;j++) {
    for(i=0;i<4;i++){
      servoPos[i] = ram.lowerlimit + 5000;
      delay(500);
    }
    for(i=0;i<4;i++){
      servoPos[3-i] = ram.upperlimit - 5000;
      delay(500);
    }
  }
  for(i=0;i<4;i++){
  ...
  delay(250);
  for(i=0;i<2512;i++){
    servoPos[0] = (int)(sin(((float)i/100))*swing + midpoint);
    servoPos[1] = (int)(sin(((float)i/100+3.14/8))*swing + midpoint);
    servoPos[2] = (int)(sin(((float)i/100+3.14/4))*swing + midpoint);
    servoPos[3] = (int)(sin(((float)i/100+3.14/8*3))*swing + midpoint);
    delay(2);
  }
  for(i=0;i<4;i++){
    servoPos[i] = midpoint;
  ...
  delay(250);
  for(i=0;i<32;i++) {
    servoPos[0] = (i&0x01) ? ram.upperlimit : ram.lowerlimit;
    servoPos[1] = (i&0x02) ? ram.upperlimit : ram.lowerlimit;
    servoPos[2] = (i&0x04) ? ram.upperlimit : ram.lowerlimit;
    servoPos[3] = (i&0x08) ? ram.upperlimit : ram.lowerlimit;
    delay(800);
  }
  for(i=0;i<4;i++){
    servoPos[i] = midpoint;
  ...
  for(j=0;j<2;j++) {
    for(i=0;i<2000;i++) {
      servoPos[0] = (int)(ram.lowerlimit + 5000 + (i%500)*swing/250);
      servoPos[1] = (int)(ram.lowerlimit + 5000 + (i%1000)*swing/500);
      servoPos[2] = (int)(ram.lowerlimit + 5000 + (i%1500)*swing/750);
      servoPos[3] = (int)(ram.lowerlimit + 5000 + (i%2000)*swing/1000);
      delay(2);
    }
  }
  for(i=0;i<4;i++){
...
void set_default_ram() {
  ...
  ram.bitrate = 115200;
  ram.spe.i = 0xffff;
  ram.lowerlimit = 10000;
  ram.upperlimit = 50000;
  ram.pivotstate = 1;
  ram.structend = 42;
  int i;
  for(i=0;i<16;i++){
